...
<script>
	...
	function getPostDetails(diidx) {
		...
		// AJAX 요청 설정
		xhr.open('GET', 'diary_detail.do?diidx=' + diidx, true);
		xhr.send();
	}
...
<body>
	...
	<form action="diary_detail.do" method="get">
		<c:forEach var="vo" items="${ didata }">

			<div class="board_box">
				<br>
				<div class="type_title">제목: ${ vo.dititle }</div>
				<div class="type_content">${ vo.dicontent }</div>
				<c:if test="${ vo.difilename ne 'no_file' }">
					<img src="resources/upload/${ vo.difilename }" width="200" />
				</c:if>
				<div class="type_date">${ vo.didate }</div>
				<div class="type_animaltype">${ vo.dianimaltype }</div>
				<div class="type_section">${ vo.disection }</div>
				<br> <input type="hidden" name="diidx" value="${ vo.diidx }">
				<input type="hidden" name="dititle" value="${ vo.dititle }">
				<input type="hidden" name="dicontent" value="${ vo.dicontent }">
				<input type="hidden" name="difilename" value="${ vo.difilename }">
				<input type="hidden" name="didate" value="${ vo.didate }"> <input
					type="hidden" name="dianimaltype" value="${ vo.dianimaltype }">
				<input type="hidden" name="disection" value="${ vo.disection }">
				<!-- 게시글 상세 정보 요청 버튼 -->
               <form action="diary_detail.do" method="get">
